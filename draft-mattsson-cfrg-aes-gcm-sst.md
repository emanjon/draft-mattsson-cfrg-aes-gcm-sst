...
{{comp1}} compares AES-GCM-SST, AES-GCM {{RFC5116}}, and ChaCha20-Poly1305 {{RFC7539}} in unicast security protocols with replay protection, where v represents the number of decryption queries. In GCM and Poly1305, the forgery probability depends on ℓ = (P_MAX + A_MAX) / 16 + 1. In AES-based algorithms, the Bernstein bound introduces a factor δ ⪅ 1 + (q + v)<sup>2</sup> ⋅ ℓ<sup>2</sup> / 2<sup>129</sup>. Notably, GCM provides no reforgeability resistance, which significantly increases the expected number of forgeries.
