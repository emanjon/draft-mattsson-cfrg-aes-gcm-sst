...
For tags with tag_length < 128 - log2(n + m + 1) bits, the expected numer of forgeries is ≈ q ⋅ 2<sup>-tag_length</sup>, which is ideal. This far outperforms GCM, where the expected number of forgeries is q<sup>2</sup> ⋅ (n + m + 1) ⋅ 2<sup>-tag_length + 1</sup> {{Multiple}}. BSI states that using an ideal MAC with a 96-bit tag length is acceptable {{BSI}}, and GCM-SST meets this standard. Achieving a comparable level of security with AES-GCM, AES-CCM, or ChaCha20-Poly1305 is nearly impossible as . These algorithms are constrained by AES's narrow 128-bit block size and the 128-bit digest size of GHASH and Poly1305. Moreover, their forgery probability depends on the message length, and they offer zero reforgeability resistance.
